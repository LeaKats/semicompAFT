...
#load the cpp code for the frailty-LTRC package
sourceCpp("frailty_LT_age_weights_brob.cpp", verbose=TRUE,rebuild = T)

# load the estimation function
estimate_all <- function(theta_init,z12,z13,z23,obs_times1,obs_times2,delta1,delta2,delta3,R=rep(0,length(obs_times1)),RecL=0,marginal_13_haz=NULL,marginal_13_grid=NULL,n_Bootstrap=0,tol=0.001,max_Iter=300,initial_pars=NULL,initial_H=NULL,seed=1,save_address=NULL,name=NULL,check_times=seq(0,max(obs_times1,obs_times2),length.out = 100),inf_f = 1)
{
  ...
  for(j in c(1:(1+n_Bootstrap)))
  {
    ...
    while(TRUE)
    {
      ...
      if(fail == F)
      {
        H012res <- rbind(H012res,H012_estimated(V))
        H013res <- rbind(H013res,H013_estimated(V))
        H023res <- rbind(H023res,H023_estimated(W))
  ...
  print(par_res)
  list(H012=H012res,H013=H013res,H023W=H023_estimated(W),H023V=H023_estimated(V),pars=par_res,convergence=conv_res)
}
...
data$V = data$V + rnorm(length(data$V),0,0.0001) #guidance from Nir in order to run the code without errors
data$W[data$W != 0] = data$W[data$W != 0] + rnorm((sum(data$W != 0)),0,0.0001) #guidance from Nir in order to run the code without errors
data$delta1<-data$delta1==1
data$delta2<-data$delta2==1
data$delta3<-data$delta3==1

#define age at relapse
data$age_at_relapce_10<-data$age_10+data$V/10

#choose the covariates at each transition
X_names01<-c("age_10","log_nodes","log_er","log_pgr","meno","size20_50","size50","hormon","chemo","grade3")
X_names02<-c("age_10","log_nodes","log_er","log_pgr","meno","size20_50","size50","hormon","chemo","grade3")
X_names12<-c("age_at_relapce_10","log_nodes","log_er","log_pgr","meno","size20_50","size50","hormon","chemo","grade3")
covar12<-as.matrix(data[,X_names01])
covar13<-as.matrix(data[,X_names02])
covar23<-as.matrix(data[,X_names12])

r<-1
#takes time....
est<-estimate_all(theta_init = 2,z12=covar12,z13=covar13,z23=covar23,
                  obs_times1 = data$V,obs_times2 = data$W,delta1=data$delta1,
                  delta2=data$delta2,delta3=data$delta3, 
                  save_address = "output",n_Bootstrap = 100,seed=r,name=r,max_Iter=300)
