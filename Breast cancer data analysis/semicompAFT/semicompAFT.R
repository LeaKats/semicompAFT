...
source("./SemicompAFT/rcpp_based_functions.R")
...
bandwidths<-function(V,W,delta1,delta2,delta3,ah01,ah02,ah12){
  ...
  sd2    <- sd(log(V))
  sd12_ev<- sd(log(W0[delta3_0==1]))
  sd12   <- sd(log(W0))
  #Find the bandwidths
  a_n1   <- ah01*c((8*sqrt(2)/3)^(1/5)*sd1_ev *(n)^(-1/5),4^(1/3)*sd1*n^(-1/3))
  a_n2   <- ah02*c((8*sqrt(2)/3)^(1/5)*sd2_ev *(n)^(-1/5),4^(1/3)*sd2*n^(-1/3))
  a_n12  <- ah12*c((8*sqrt(2)/3)^(1/5)*sd12_ev*(sum(delta1_0==1))^(-1/5),4^(1/3)*sd12*sum(delta1_0==1)^(-1/3))
...
estimation_with_frailty<-function(X01,X02,X12,V,W,delta1,delta2,delta3,zeta_beta=50,zeta_h=0.01,initial_sigma=2,stop_iter_num=1000,conv_betas_bound=0.00001,conv_Hs_bound=0.0001,conv_sigma_bound=0.0001,B,print)
{
  ...
  band  <- bandwidths(V=data$V,W=data$W,delta1=data$delta1,delta2=data$delta2,delta3=data$delta3,ah01=ah01,ah02=ah02,ah12=ah12)
  a_n1  <- band[1,]
  a_n2  <- band[2,]
  a_n12 <- band[3,]
  ...
  sigma_m_plus1 <- sigma_opt(init_sigma = sigma_m,sumE1=posterior_expectations_m_plus1[["sumE1"]],sumE2=posterior_expectations_m_plus1[["sumE2"]],n=n)
  ...
  while(conv==F){
    ...
    if ((conv_beta01&conv_beta02&conv_beta12)==F){
    l12<- lbfgs(l_s_m_12_f.CPP(),grad_beta12_f.CPP(),vars=c(beta_hat_12_m), environment =env12,invisible = 1,linesearch_algorithm = "LBFGS_LINESEARCH_BACKTRACKING")
  ...
  while (length(sigma_pert)<B){
    ...
    # estimating Betas
    beta_hat_01_m <-  lbfgs(l_s_m_01_02pert_f.CPP(),grad_beta01_02pert_f.CPP(),env=env01,vars=as.numeric(tau_conv[ grep("beta01_conv",names(tau_conv))]),invisible = T)$par
    beta_hat_02_m <-  lbfgs(l_s_m_01_02pert_f.CPP(),grad_beta01_02pert_f.CPP(),env=env02,vars=as.numeric(tau_conv[ grep("beta02_conv",names(tau_conv))]),invisible = T)$par
    ...
    while(conv==F){
      ...
      start<-Sys.time()
      if ((conv_beta01&conv_Hs)==F) {
        H0_01_obs_m_plus1 <- H0_hat_01_02_observed_perturbed_f(beta=beta_hat_01_m_plus1,gamma=c(E1_m_plus1),a_n=a_n1,X=X01,V=V,delta=delta1,G)
      } else {H0_01_obs_m_plus1 <-H0_01_obs_m}
      end<-Sys.time()
      #print(end-start)
      ...
      start<-Sys.time()
      if ((conv_beta02&conv_Hs)==F) {
        H0_02_obs_m_plus1<- H0_hat_01_02_observed_perturbed_f(beta=beta_hat_02_m_plus1,gamma=c(E1_m_plus1),a_n=a_n2,X=X02,V=V,delta=delta2,G)
      } else {H0_02_obs_m_plus1<-H0_02_obs_m}
      end<-Sys.time()
      #print(end-start)
      ...
      start<-Sys.time()
      if ((conv_beta12&conv_Hs)==F) {
        H0_12_obs_m_plus1<-H0_hat_12_observed_perturbed_f_new (beta=beta_hat_12_m_plus1,gamma=c(E1_m_plus1),a_n12=a_n12,X12=X12,V=V,W=W,delta1 = delta1,delta3 = delta3,G)
      } else {H0_12_obs_m_plus1<-H0_12_obs_m}
      end<-Sys.time()  
      ...
      print(paste("Bootstrap iteration=",i,length(sigma_pert)))
      pert_all           <- rbind(pert_all,
                                  cbind(sigma_pert0,t(b01_pert0),t(b02_pert0),t(b12_pert0),boot_time0,iter0,i)
                                  )
  ...
  return(list(results=cbind(coefficient=est,ESE=c(pert_se)),
              est=est_par,
              ESE=c(pert_se),
              all_boot_res=pert_all) )
}
